...
export class UpdateProductUseCase implements UseCase<Input, Output> {
  ...
  async execute(input: Input): Promise<Output> {
    ...
    // 1. Diff + persistência das receitas e custos adicionais — SOMENTE produção própria
    ...
    if (isOwnProduction) {
      ...
      if (additionalCostsUsage.length || hasRecipeLinks) {
    ...
    product.update({
      ...
      imagePath,
      // Sem lote/tela de Lotes, editar o produto é o único jeito de corrigir o
      // estoque manualmente — usa o valor informado se vier, senão mantém o atual.
  ...
  /**
   * Faz o diff dos vínculos com receitas (só cria/remove, não há "quantidade"
   * nesse vínculo — quem tem quantidade é o item dentro da própria receita).
   * Recalcula o custo de TODAS as receitas vinculadas a cada save, mesmo as
   * que não mudaram, porque o preço das matérias-primas de uma receita pode
   * ter mudado desde o último save do produto.
   */
